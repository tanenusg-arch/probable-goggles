...
const config = {
  ...
  FIRESTORE_API_KEY: cfg.FIRESTORE_API_KEY || process.env.FIRESTORE_API_KEY || '',
  PROOF_CHANNEL_ID: cfg.PROOF_CHANNEL_ID || '',
  REPORT_CHANNEL_ID: cfg.REPORT_CHANNEL_ID || '',
  NOTICE_CHANNEL_ID: cfg.NOTICE_CHANNEL_ID || '',
  ADMIN_PASSWORD: cfg.ADMIN_PASSWORD || process.env.ADMIN_PASSWORD || '',
...
class FirestoreDatabase {
  ...
  async atomicIncrement(telegramId, field, amount) {
    const path = `users/${telegramId}`;
    const payload = {
      updateTransforms: [{
        fieldPath: field,
        increment: { doubleValue: Number(amount) }
      }]
    };
  ...
  async updateBalance(telegramId, amount) {
    return this.atomicIncrement(telegramId, 'balance', amount);
  }
  ...
  async updateReferralBalance(telegramId, amount) {
    return this.atomicIncrement(telegramId, 'referral_balance', amount);
  }
  ...
  async approveDeposit(depositId, adminNote = '') {
    ...
    const added = await this.updateBalance(parseInt(deposit.user_id), deposit.amount);
  ...
  async rejectWithdrawal(withdrawalId, reason = '', adminId = null) {
    ...
    const params = { updateMask: { fieldPaths: Object.keys(fields) } };
    await this._request('PATCH', path, payload, params);
    await this.updateBalance(parseInt(w.user_id), parseFloat(w.amount || 0) + parseFloat(w.fee || 0));
    return true;
  }
  ...
  bot.on('message', async (msg) => {
    ...
    if (state === STATES.PAYMENT_TXN_ID) {
      ...
      if (result) {
        ...
        const placed = await placeOrderAfterVerification(bot, db, apiClient, chatId, userId, method, reference, verifiedAmount, msg.from.first_name);
        if (placed) await db.recordTransactionUse(reference, userId, verifiedAmount);
        return;
      } else {
        await bot.deleteMessage(chatId, verifyingMsg.message_id);
    ...
    if (state === STATES.DEPOSIT_TXN_ID) {
      ...
      if (result) {
        ...
        const depositId = await db.createDeposit(userId, method, amount, 'ETB', '', reference);
        if (!depositId) {
          await bot.editMessageText(`${EMOJIS.CROSS} Could not create the deposit record. Your payment was not credited automatically. Please contact support.`, { chat_id: chatId, message_id: verifyingMsg.message_id, parse_mode: 'HTML', reply_markup: getMainInlineKeyboard() });
          clearState(chatId);
          return;
        }
        const approved = await db.approveDeposit(depositId, `Auto-approved Ref: ${reference}`);
        if (!approved) {
          await bot.editMessageText(`${EMOJIS.CROSS} Payment was verified but could not be credited automatically. Please contact support with reference <code>${reference}</code>.`, { chat_id: chatId, message_id: verifyingMsg.message_id, parse_mode: 'HTML', reply_markup: getMainInlineKeyboard() });
          clearState(chatId);
          return;
        }
    ...
    if (state === STATES.ADMIN_LOGIN) {
      if (config.ADMIN_PASSWORD && text === config.ADMIN_PASSWORD) {
        setState(chatId, STATES.ADMIN_MAIN);
